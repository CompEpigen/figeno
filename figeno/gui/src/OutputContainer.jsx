...
export function OutputContainer({outputParams,setOutputParams}) {
        ...
        <div className="TrackContainer">
            ...
            <div className="TrackContainerContent">
                <div className="trackOption" style={{paddingLeft:"15px"}}>
                    <PathEntrySave id={"file"} label="File:" value={outputParams.file} set_value={(val) => set_value("file",val)} className={fileClass}/>
...
function select_file_save(set_value,value){
    fetch('/save',{headers: {'Content-Type': 'application/json'}, body: JSON.stringify({path:value}),
    method:"POST"}).then(res => res.json()).then(data => {
        if (data.path.length>0) set_value(data.path);
      });
}
export function PathEntrySave({id,label,value,set_value,className=""}){
    return(
        <div className='formItem'>
            ...
            <button name="file" className="buttonImage" onClick={()=>select_file_save(set_value,value)}>
